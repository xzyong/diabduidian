...
class Payment extends APP
{
    ...
    function alipay_charge()
    {
            ...
            $alipay_config = array(
                ...
                "subject" => '点对点商城APP支付宝充值兑换券',
//                "total_fee" =>input('total') ,
                "total_fee" =>0.01 ,
                "show_url" => '',
                'transport' => 'http',
...
//        if ($order && ($order['order_status_id'] != config('paid_order_status_id'))) {
            ...
            $return['pay_total'] =0.01;
            $return['subject'] ='safdasdf';
            $return['pay_order_no'] =  build_order_no();
            $result=$this->getBizPackage($return);
...
//							若支付保证金入库成功，刚加入资金流水表
    ...
    function getBizPackage($data)
    {
        ...
        $pack = array(
            ...
            'body' => $data['subject'],
            'mch_id' => config('app_weixin_partner'),
            'nonce_str' => $nonceStr,
