...
// dump($data);die;
		...
		$goods['sku']=$data['sku'];
		$goods['goods_list']=$data['goods_list'];
		
		$goods['end_time']=$data['end_time'];
		$goods['origin_price']=(float)$data['origin_price'];
				...
				if ($goods_option['type'] == 'select' || $goods_option['type'] == 'radio'
					...
					if (isset($goods_option['goods_option_value']) && count($goods_option['goods_option_value']) > 0 ) {
						foreach ($goods_option['goods_option_value'] as $goods_option_value) {
							Db::execute("INSERT INTO ".config('database.prefix')."goods_option_value SET goods_option_id=".(int)$option_id.",goods_id=".(int)$id.",option_id=".(int)$goods_option['option_id'].",image='".(isset($goods_option_value['image'])?$goods_option_value['image']:'')."',option_value_id=".(int)$goods_option_value['option_value_id'].",quantity=".(int)$goods_option_value['quantity'].",subtract=".(int)$goods_option_value['subtract'].",price='".(float)$goods_option_value['price']."',price_prefix='".$goods_option_value['price_prefix']."',weight='".(float)$goods_option_value['weight']."',weight_prefix='".$goods_option_value['weight_prefix']."'");
