...
class CustomGoods extends GoodsBase{
	protected function _initialize(){
		parent::_initialize();
		$this->assign('breadcrumb1','兑换商品');
		$this->assign('breadcrumb2','兑换商品管理');
	}
...
//新增商品
	 public function add(){
		...
		if(request()->isPost()){
			...
			if($return){
				...
				$this->success('新增成功！',url('CustomGoods/index'));			
			}else{
				$this->error('新增失败！');
			}
		}
		...
		$this->assign('action', url('CustomGoods/add'));
	 ...
	 public function edit_general(){
		...
		if(request()->isPost()){
			...
			try{
				...
				return $this->success('更新成功！',url('CustomGoods/index'));
	...
	function copy_goods(){
		...
		if($id){		
			...
			$data['redirect']=url('CustomGoods/index');	
	...
	function del(){
		...
		if($r){
			...
			$this->redirect('CustomGoods/index');
			
		}else{
			
			return $this->error('删除失败！',url('CustomGood/index'));
		}		
