	...
	public function add(){
		...
		if(request()->isPost()){
			...
			if($id){
				...
				User::get_logined_user()->storage_user_action('添加了后台菜单，'.$data['title']);
	...
	function edit(){
		...
		if(request()->isPost()){
			...
			if($r){
				...
				User::get_logined_user()->storage_user_action('修改了后台菜单，'.$data['title']);
	...
	function del(){
		...
		if(request()->isPost()){
			...
			if(Db::name('Menu')->where('id',$id)->delete()){
				...
				User::get_logined_user()->storage_user_action('删除了后台菜单，id='.$id);
