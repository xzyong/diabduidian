...
//通过地区的名称取地区的id
function get_area_id_by_name($area_name,$parentid=0,$area_deep=1){
	...
	/*艺拍项目临时代码,若地址库中没有找到省市区名字，则添加上一个 开始*/
	if(!isset($area_list[$area_name])){
		$newarea = new Area([
				'area_name' => $area_name,
		...
		$result = $newarea->save();
		cache('area_name_list',null);

		return get_area_id_by_name($area_name,$parentid,$area_deep);
	}

	/*艺拍项目临时代码,若地址库中没有找到省市区名字，则添加上一个 结束*/
