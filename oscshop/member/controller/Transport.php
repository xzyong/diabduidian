...
use think\Db;
use osc\member\model\Transport as TransportModel;
...
class Transport extends AdminBase{
	...
	function clone_data(){
		...
		User::get_logined_user()->storage_user_action('复制了运费模板');
	...
	function save(){
		...
		User::get_logined_user()->storage_user_action('更新了运费模板');
	...
	function del(){
		...
		if($model->delTansport(array('id'=>$id))){
			User::get_logined_user()->storage_user_action('删除了运费模板');
		   $this->redirect('Transport/index');          
		}else{
			$this->error('操作失败');
		}
